				...
				if (datos.Images) {
					...
					if (images.boxart) {
						...
						for(var i = 0; i < images.boxart.length; i++) {
							...
							//descargamos las miniaturas y las colocamos como originales
							if (images.boxart[i].side[0] === 'front') {
								var img = syncRequest('GET', boxart.thumb);
								var commonRoute = 'img/boxartfront/' + boxart.thumb.substr(boxart.thumb.lastIndexOf('/'), boxart.thumb.length);
								fs.writeFileSync('../public/' + commonRoute, img.body);
								boxart.url = commonRoute;
								json.images.boxart.front = boxart;
							}
							else if (images.boxart[i].side[0] === 'back') {
								var img = syncRequest('GET', boxart.thumb);
								var commonRoute = 'img/boxartback/' + boxart.thumb.substr(boxart.thumb.lastIndexOf('/'), boxart.thumb.length);
								fs.writeFileSync('../public/' + commonRoute, img.body);
								boxart.url = commonRoute;
